...
llm_chain = LLMChain(llm=gpt4, prompt=PromptTemplate.from_template(PROMPT), verbose=True)
...
if st.session_state.llm_on:
    if not st.session_state.llm_output:
        ...
        st.session_state.llm_output = llm_chain.run(input)   # return output string from LLM
        
# convert output string to dataframe
str_buffer = StringIO(st.session_state.llm_output)
